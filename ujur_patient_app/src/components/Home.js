...
import { fetchCustomerDoctors, fetchCustomerlatestAppointment, fetchCustomerHospital  } from '../urls/urls';
...
const Home = () => {
  ...
  const [hospitalsResponse, hospitalsError, hospitalsLoading, hospitalsFetch] = useAxios();
 ...
 const fetchHospitalsFunc = () => {
  hospitalsFetch(fetchCustomerHospital({
    pageNumber:4
  }));
  };
 const fetchDoctorsFunc = () => {
  doctorsFetch(fetchCustomerDoctors({
    pageNumber:4
     ...
     useEffect(()=>{
      ...
      useEffect(()=>{
         if(fetchAppointmentResponse?.result == "success" && fetchAppointmentResponse?.data){
        ...
        <Link to={`/hospital-overview/${each?.id}`} className="link-dark">
          <div className="card bg-white border-0 rounded-4 shadow-sm overflow-hidden">
            ...
            <div className="card-body small p-3 osahan-card-body">
              ...
              <p className="card-text text-muted small m-0">{each?.description && each.description.length > 30 ? each.description.substring(0, 30) + '...' : each.description}</p>
              <p className='mt-3'><span className="mdi mdi-star text-warning me-1" />
                4.9 (5,380)</p>
            </div>
          </div>
        </Link>
...
<div className="bg-white shadow-sm">
  ...
  <div className="col">
    <div className="bg-white text-center rounded-4 p-2 shadow-sm">
      <Link to="/search-doctor" className="link-dark" style={{textDecoration:"none"}}>
  ...
  <div className="col">
    <div className="bg-white text-center rounded-4 p-2 shadow-sm" >
      <Link to="/view-appointments" className="link-dark" style={{textDecoration:"none"}}>
  ...
  <div className="col">
    <div className="bg-white text-center rounded-4 p-2 shadow-sm" >
      <Link to="/lab-reports" className="link-dark" style={{textDecoration:"none"}}>
        <img
          src={doctorImg3}
          alt=""
  ...
  <div className="col">
    <div className="bg-white text-center rounded-4 p-2 shadow-sm" >
      <Link to="/favorite-doctor" className="link-dark" style={{textDecoration:"none"}}>
  ...
  <Link to={`/appointment-details/${latestAppointment.id}`} class="link-dark" style={{textDecoration:"none"}}>
                           <div class="bg-white d-flex align-items-center gap-3 p-3 mb-1 shadow-sm">
                              <img src={test_url_images + latestAppointment?.doctor?.profile_picture} alt="" class="img-fluid rounded-4 voice-img" />
                              <div>
                                 <h6 class="mb-1">Dr. {latestAppointment?.doctor?.full_name}</h6>
                                 <p class="text-muted mb-2">{latestAppointment?.doctor?.education}</p>
                                 <p class="text-muted m-0"><span class="mdi mdi-calendar-month text-primary me-1"></span>
                                 {moment(latestAppointment?.date_appointment).format('DD-MM-YYYY')}
...
{doctorsData.map((each)=>{return( <div className="col">
        ...
        >
          ...
          />
          <div className="card-body small p-3 osahan-card-body">
            <h6 className="card-title fs-14 mb-1">Dr. {each?.full_name}</h6>
            <p className="card-text text-muted mb-1">{each?.education}</p>
            <div className="d-flex align-items-center gap-3 small mb-3">
              <div>
                <span className="mdi mdi-star text-warning me-1" />
                {each?.avg_reviews ? each?.avg_reviews : "N/A"} ({each?.total_reviews && each?.total_reviews})
              </div>
              <div>
            ...
            <h6 className="mb-0">
              Rs {each?.doctor_slots[0]?.morning_slots_price}<span className="text-muted small ms-1">Inc.Gst</span>
            </h6>
          </div>
